...
def saveData(path, trials):
    ...
    with open(path, 'w') as csvfile:
        ...
        for trial in trials:
            #file.writerow([str(i).encode('utf-8') for i in trial]) #changed from unicode to str
            file.writerow([str(i) for i in trial]) #changed from unicode to str
...
def invert(d):
    _a = list(reversed(list(d.values()))) # changed the call
    return {k: v for k, v in zip(d.keys(), _a)}
...
#    selfnegopo = dict(negPos.items() + selfOther.items())
    negopoself = dict(tuple(negPos.items()) + tuple(otherSelf.items())) # added touple type
    selfnegopo = dict(tuple(negPos.items()) + tuple(selfOther.items())) # added touple type
...
def runExperiment(pause, instructionOrder, blockOrder):
    ...
    data = []
    trialCount = 0
    mapping = itertools.zip_longest(instructionOrder, blockOrder) # itertools.izip_longest(instructionOrder, blockOrder)
    ...
    for instr, block in mapping:
        ...
        if instr is None:
            ...
            pause(text=instr, font='Arial', wrapWidth=1.5, height=0.06)
